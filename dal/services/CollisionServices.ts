...
export class CollisionServices implements ICollisionServices {
  ...
  public getCollisions = (): GetCollisionDTO[] => {
    const collisionResponse = this.db.map((element) =>
      mapResponseToDTO<GetCollisionDTO, Collision>(element, {
        id: "id",
        ksi: "ksi",
        location: "location",
  ...
  public getOneCollision = (id: number): GetCollisionDTO => {
    const collisionToMap: Collision | undefined = this.db.find(
      (element) => element.id === id
    );
    let collisionResponse;
    if (collisionToMap !== undefined) {
      collisionResponse = mapResponseToDTO<GetCollisionDTO, Collision>(
        collisionToMap,
        {
          id: "id",
          ksi: "ksi",
